...
export type UTXO = {
  txid: string;       // ID транзакции, содержащей этот выход
  vout: number;       // Индекс выхода в транзакции
  value: string;      // Количество монет
  scriptPubKey?: string; // Скрипт блокировки
  confirmations?: number; // Количество подтверждений
  address?: string;   // Адрес, которому принадлежит UTXO
};